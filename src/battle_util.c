...
//   normal  fight   flying  poison  ground  rock    bug     ghost   steel   mystery fire    water   grass  electric psychic ice     dragon  dark    fairy
    ...
    {X(1.0), X(1.0), X(1.0), X(1.0), X(2.0), X(2.0), X(1.0), X(1.0), X(1.0), X(1.0), X(2.0), X(0.5), X(0.5), X(1.0), X(1.0), X(0.0), X(0.5), X(1.0), X(1.0)}, // water
...
static bool32 ShouldChangeFormHpBased(u32 battler)
{
    ...
    static const u16 forms[][4] =
    {
        ...
        {ABILITY_SHIELDS_DOWN, SPECIES_GOLEM, SPECIES_GOLEM_SHIELDS, 2},
        ...
        {ABILITY_SCHOOLING, SPECIES_EXEGGCUTE_SCHOOL, SPECIES_EXEGGCUTE, 4},
    };
    u32 i;
...
u8 AbilityBattleEffects(u8 caseID, u8 battler, u16 ability, u8 special, u16 moveArg)
{
    ...
    case ABILITYEFFECT_ON_SWITCHIN: // 0
        ...
        case ABILITY_JET_STREAM:
            if (!gSpecialStatuses[battler].switchInAbilityDone)
            {
                gBattleCommunication[MULTISTRING_CHOOSER] = MULTI_SWITCHIN_JET_STREAM;
                gSpecialStatuses[battler].switchInAbilityDone = 1;
        ...
        if (gBattleMons[battler].hp != 0)
        {
            ...
            case ABILITY_TANK_SHELL:
                if (WEATHER_HAS_EFFECT
                 ...
                 && !(gStatuses3[battler] & STATUS3_HEAL_BLOCK))
                {
                    BattleScriptPushCursorAndCallback(BattleScript_TankShellActivates);
                    gBattleMoveDamage = gBattleMons[battler].maxHP / (gLastUsedAbility == ABILITY_TANK_SHELL ? 16 : 8);
                    if (gBattleMoveDamage == 0)
                        gBattleMoveDamage = 1;
                    gBattleMoveDamage *= -1;
                    effect++;
    ...
    case ABILITYEFFECT_MOVE_END_ATTACKER: // Same as above, but for attacker
        switch (gLastUsedAbility)
        {
        case ABILITY_POISON_TOUCH: // May have to duplicate Static and Flame Body to this area for same Poison Touch effect
            if (!(gMoveResultFlags & MOVE_RESULT_NO_EFFECT)
             && gBattleMons[gBattlerTarget].hp != 0
...
static u32 CalcMoveBasePowerAfterModifiers(u16 move, u8 battlerAtk, u8 battlerDef, u8 moveType, bool32 updateFlags)
{
    ...
    if (gFieldStatuses & STATUS_FIELD_MISTY_TERRAIN && moveType == TYPE_DRAGON && IsBattlerGrounded(battlerAtk) && !(gStatuses3[battlerDef] & STATUS3_SEMI_INVULNERABLE))
        MulModifier(&modifier, UQ_4_12(0.5));
...
static u32 CalcDefenseStat(u16 move, u8 battlerAtk, u8 battlerDef, u8 moveType, bool32 isCrit, bool32 updateFlags)
{
    ...
    // hail def boost for ice types (apparently removing an exclamation was all it took)
...
static void MulByTypeEffectiveness(u16 *modifier, u16 move, u8 moveType, u8 battlerDef, u8 defType, u8 battlerAtk, bool32 recordAbilities)
{
    ...
    else if ((moveType == TYPE_FIGHTING || moveType == TYPE_NORMAL) && defType == TYPE_GHOST && (gBattleMons[battlerDef].status2 & STATUS2_FORESIGHT || gStatuses3[battlerDef] & STATUS3_MIRACLE_EYED) && mod == UQ_4_12(0.0))
    {
        mod = UQ_4_12(1.0);
    }
    else if ((((moveType == TYPE_FIGHTING || moveType == TYPE_NORMAL) && defType == TYPE_GHOST) || (moveType == TYPE_PSYCHIC && defType == TYPE_DARK) || (moveType == TYPE_GROUND && defType == TYPE_FLYING)) && GetBattlerAbility(battlerAtk) == ABILITY_SCRAPPY && mod == UQ_4_12(0.0))
    {
        mod = UQ_4_12(1.0);
        if (recordAbilities)
    ...
    if (moveType == TYPE_PSYCHIC && defType == TYPE_DARK && (gBattleMons[battlerDef].status2 & STATUS2_FORESIGHT || gStatuses3[battlerDef] & STATUS3_MIRACLE_EYED) && mod == UQ_4_12(0.0))
        mod = UQ_4_12(1.0);
    if (gBattleMoves[move].effect == EFFECT_FREEZE_DRY && defType == TYPE_WATER)
        mod = UQ_4_12(2.0);
    if (moveType == TYPE_GROUND && defType == TYPE_FLYING && IsBattlerGrounded(battlerDef) && mod == UQ_4_12(0.0))
        mod = UQ_4_12(1.0);
    if (moveType == TYPE_GROUND && defType == TYPE_FLYING && (gBattleMons[battlerDef].status2 & STATUS2_FORESIGHT || gStatuses3[battlerDef] & STATUS3_MIRACLE_EYED) && mod == UQ_4_12(0.0))
        mod = UQ_4_12(1.0);
...
void UndoFormChange(u32 monId, u32 side)
{
    ...
    static const u16 species[][2] = // changed form id, default form id
    {
        ...
        {SPECIES_SUDOWOODO_BROKEN, SPECIES_SUDOWOODO},
        ...
        {SPECIES_EXEGGCUTE_SCHOOL, SPECIES_EXEGGUTOR},
    };
